...
public class FileUploadSession
{
    ...
    public async void CompleteSession(Guid sessionCorrelationId)
    {
        ...
        try
        {
            ...
            if (IsCompleted)
            {
                ...
                using(var outputStream=File.Create(Path.Combine("Uploads", FileName))){}
    ...
    public static async Task<FileUploadSession> Get(Guid sessionCorrelationId)
    {
        ...
        var session = new FileUploadSession(sessionCorrelationId, sessionData?.SessionFileName??"", sessionData?.SessionTotalParts??0);
        await session.Load();
        return session;
    }
