...
public class FileUploadSession
{
    ...
    public async void AddPart(FilePart filePart)
    {
        try
        {
            ...
            _fileParts.Add(filePart);
            //save file to disk
            var fPath = Path.Combine("Uploads/Temp", SessionCorrelationId.ToString(), filePart.PartNumber.ToString() + ".part");
            await File.WriteAllBytesAsync(fPath, filePart.Content);
    ...
    public async void CompleteSession(Guid sessionCorrelationId)
    {
        ...
        try
        {
            ...
            {
                //create final file
                using var finalFilePath = Path.Combine("Uploads", FileName);
                //write all parts to final file
                foreach (var part in _fileParts)
                {
                    await File.WriteAllBytesAsync(finalFilePath,part.Content);
                }
